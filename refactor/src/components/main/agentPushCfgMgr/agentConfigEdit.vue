        ...
        <el-row>
          <el-col :lg="{span:10}" :md="{span:12}" :sm="{span:18}" :xs="{span:24}">
            <el-form-item label="代理商" prop="agentCode">
              <el-col :span="20">
                <el-select v-model="formData1.agentCode" placeholder="请选择">
                  <el-option
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
      ...
      let agentCountId = this.$route.query.agentCount;
      ApiService.actCheck.agentFtpDetail({id:agentCountId}).then(res => {
        ...
        this.formData1.agentCode=this.formData1.agentCode+' - '+this.formData1.agentName;
      });
    },
    methods: {
      onSave(formData) {
        this.$refs[formData].validate((valid) =>{
          ...
          if(valid){
            ...
            // 保存代理商新增配置结果
            ApiService.actCheck.editAgentFtp(objtemp).then(res => {
              if(res.code==0){
