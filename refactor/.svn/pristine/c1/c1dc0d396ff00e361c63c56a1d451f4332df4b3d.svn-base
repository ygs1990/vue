  ...
  <div class="content">
    ...
    <div class="handle-box">
      <el-form :inline="true" ref="formInline" :model="formInline" class="demo-form-inline">
        <el-row>
          <el-col :span="8">
            <el-form-item label="商户:" prop="agentCode">
              <el-select v-model="formInline.agentCode" filterable placeholder="请输入名称或商户编号">
                <el-option
    ...
    <div class="handle-box">
      <router-link to="/addMchnt">
        <el-button type="primary">新增商户</el-button>
      </router-link>
    </div>
    <el-table :data="tableData" border style="width: 100%" ref="multipleTable">
      <el-table-column prop="code" label="商户编号" width="220"></el-table-column>
      <el-table-column prop="name" label="商户名称"></el-table-column>
      <el-table-column prop="mchntTxName" label="开通产品"></el-table-column>
      <el-table-column prop="isSelfOperated" label="是否自营" :formatter="isSelfFormat"></el-table-column>
      <el-table-column prop="status" label="商户状态" :formatter="statusFormat"></el-table-column>
      ...
      <el-table-column label="操作" width="260">
        <template scope="scope">
          <span class="wrapper">
            ...
            <el-button size="small" type="info" @click="handleDelete(scope.$index, scope.row)">重置秘钥</el-button>
          </span>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      insDetail(index, row,disabled) {
        this.$router.push({path: 'detailMchnt', query: {id: row.id,disabled:disabled}});
      },
      handleDelete(index, row) {
        this.$confirm('确定重置秘钥?', '提示', {
          ...
          ApiService.mchnt.reMchntKey({code: row.code,id:row.id}).then(data =>{
            if (data.code === 0) {
              // 点击确定按钮时，弹出如下信息及操作
              this.$message({
                type: 'success',
      ...
      freezeIns(index, row) {
        let _this = this;
        _this.$confirm('是否确定' + row.operation + '该商户？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() =>{
          // 重置通道状态及操作
          ApiService.mchnt.reBlockeMchnt({
            id: row.id,
            status: row.statusId
          }).then(data =>{
            if (data.code === 0) {
